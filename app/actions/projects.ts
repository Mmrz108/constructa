...
export async function rejectProject(id: number, reason: string) {
  ...
  if (!rejectReason) throw new Error("A rejection reason is required")
...
export async function finishProject(id: number) {
  ...
  await db
    .update(project)
    .set({ status: "completed", rejectReason: null })
    .where(and(eq(project.id, id), eq(project.orgId, orgId)))
  revalidateProject(id)
}

/** Resume a rejected project (Continue work). */
export async function continueProject(id: number) {
  const { orgId } = await requireContext()
  await db
    .update(project)
    .set({ status: "active", rejectReason: null })
    .where(and(eq(project.id, id), eq(project.orgId, orgId)))
  revalidateProject(id)
}
